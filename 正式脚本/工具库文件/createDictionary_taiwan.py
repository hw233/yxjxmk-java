    ...
    for table in data.sheets():
        ...
        try:
            ...
            for r in range(nrows):
                if ncols<2:
                    continue
                key = str(table.cell_value(r,0))
                val = str(table.cell_value(r,1))
                if len(key.strip())>0 and len(val.strip())>0 :
                    transDict[key] = val
        except Exception,e:  
            print Exception,":",e
            os.system('pause')
...
def table2Dict(table):
    ...
    for r in range(nrows):
        ...
        for c in range(ncols):
            ...
            if objType=='STRING':
                strCellValue = str(table.cell_value(r,c))
