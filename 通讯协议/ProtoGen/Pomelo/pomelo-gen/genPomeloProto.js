    ...
    if (handlers[fullHandlerName] === undefined) {
        ...
        for (var keyRpc in reflectT.children) {
            ...
            if (isRequest(rpcName)) {
                ...
                try {
                    var fullPushHandler = reflectT.parent.name + '.' + pushHandler;
...
function genConstuctor(indent, classInfo) {
    ...
    if (classInfo.type === 'socket') {
        ...
        for (var i in classInfo.events) {
            ...
            code += genIndent(indent + 1) + "Pomelo.DotNetClient.EventTypes.RegistPushType(\"" + event.route + "\", typeof(" + event.requestType + "));\n"
        }
        for (var i in classInfo.requests) {
            var request = classInfo.requests[i];
            code += genIndent(indent + 1) + "Pomelo.DotNetClient.EventTypes.RegistRequestType(\"" + request.route + "\", typeof(" + request.requestType + "), typeof(" + request.responseType + "));\n"
        }
        for (var i in classInfo.notifies) {
            var notify = classInfo.notifies[i];
            code += genIndent(indent + 1) + "Pomelo.DotNetClient.EventTypes.RegistNotifyType(\"" + notify.route + "\", typeof(" + notify.requestType + "));\n"
        }
        code += genIndent(indent) + "}\n";
        ...
        code += genMethodBegin(indent, classInfo.name, [
            ...
            { type: 'ProtobufSerializer', name: 'serializer' }
        ]);
...
function genRequest(indent, classInfo, request) {
    ...
    code += genIndent(indent + 1) + request.requestType + ' request = new ' + request.requestType + '();\n';
    ...
    code += genIndent(indent + 1) + '_socket.request<' + respName + '>(request, (' + respName + ' rsp, out int s2c_code, out string s2c_msg) => {\n';
...
function genNotify(indent, classInfo, notify) {
    ...
    code += genIndent(indent + 1) + notify.requestType + ' notify = new ' + notify.requestType + '();\n';
    ...
    code += genIndent(indent + 1) + '_socket.notify("' + notify.route + '", _serializer, notify);\n';
    code += genMethodEnd(indent);
    return code;
}
...
function genEvent(indent, classInfo, event) {
    ...
    if (classInfo.type == 'socket') {
        ...
        code += genIndent(indent + 1) + '_socket.on_push("' + event.route + '", stream =>  {\n';
...
function genLuaRequest(indent, classInfo, request) {
    ...
    if (request.params.length == 0) {
        code += genIndent(indent + 1) + 'Socket.Request("' + request.route + '", input,function(stream)\n';
    }
    else {

        code += genIndent(indent + 1) + 'Socket.Request("' + request.route + '", ZZBase64.encode(input:SerializeToString()),function(stream)\n';
    }

    code += genIndent(indent + 2) + 'stream = ZZBase64.decode(stream)\n';
    code += genIndent(indent + 2) + 'local res = ' + firstLowerCase(classInfo.name) + '_pb.' + request.responseType + '()\n';
    code += genIndent(indent + 2) + 'res:ParseFromString(stream)\n';
    ...
    code += genIndent(indent + 1) + 'end,option)\n';
...
function genLuaNotify(indent, classInfo, notify) {
    ...
    //}
    if (notify.params.length == 0) {
        code += genIndent(indent + 1) + 'Socket.Notify("' + notify.route + '", input)\n';
    }
    else {
        code += genIndent(indent + 1) + 'Socket.Notify("' + notify.route + '",ZZBase64.encode(input:SerializeToString()))\n';
...
function genLuaEvent(indent, classInfo, event) {
    ...
    code += genIndent(indent + 1) + 'Socket.On("' + event.route + '", function(stream) \n';
    code += genIndent(indent + 2) + 'stream = ZZBase64.decode(stream) \n';
    code += genIndent(indent + 2) + 'local res = ' + firstLowerCase(event.handler) + '_pb.' + event.requestType + '() \n';
    code += genIndent(indent + 2) + 'res:ParseFromString(stream) \n';
    code += genIndent(indent + 2) + 'if(res.s2c_code == 200) then \n';
    code += genIndent(indent + 3) + 'Pomelo.' + classInfo.name + '.last' + event.requestType + ' = res\n';
    //   code += genIndent(indent+3)+'Pomelo.MasteryHandler.lastUpqualityResponse = res \n';
    code += genIndent(indent + 3) + 'Socket.OnRequestEnd("' + event.route + '", true) \n';
    code += genIndent(indent + 3) + 'cb(nil,res) \n';
    code += genIndent(indent + 2) + 'else \n';
    code += genIndent(indent + 3) + 'local ex = {} \n';
    code += genIndent(indent + 3) + 'if(res.s2c_code) then \n';
    ...
    code += genIndent(indent + 3) + 'else \n';
    code += genIndent(indent + 4) + 'ex.Code = 501 \n';
    code += genIndent(indent + 4) + 'ex.Message = "[LuaXmdsNetClient] ' + event.route + ' decode error!!" \n';
    code += genIndent(indent + 3) + 'end \n';
    code += genIndent(indent + 3) + 'Socket.OnRequestEnd("' + event.route + '", false,ex.Code,ex.Message) \n';
    code += genIndent(indent + 3) + 'cb(ex,nil) \n';
    code += genIndent(indent + 2) + 'end \n';
    code += genIndent(indent + 1) + 'end) \n';
